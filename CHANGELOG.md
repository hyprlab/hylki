...
changes of its own: see the 1.41.1 section below for what is in it.
...
A Mailfence account shows its mail, replies through Proton Bridge join
their conversation, and the theme chooser shows each theme as a window.

- **Mailfence accounts no longer show an empty list** (#226). The server's
  reply to the message-list fetch could not be read by the IMAP parser on
  either of the two ways the app asks, ENVELOPE or raw headers, because
  both carried the list-preview items (`BODY.PEEK[1]<0.N>` and
  `BODY.PEEK[1.MIME]`) Mailfence is known to answer badly. The retry after
  an unreadable reply now walks a ladder instead of flipping one switch:
  ENVELOPE with previews, headers with previews, ENVELOPE without
  previews, headers without previews. Once the preview items are found to
  be the trouble, every preview read for the account is skipped for the
  session, since a rejected reply leaves the connection unusable. Only
  parse errors step the ladder; a dropped connection is retried once as it
  was. The backfill steps the same ladder.
- **A readable error instead of a wall of numbers.** The status bar showed
  the parser's error as it came, the whole reply as byte values. It now
  says the reply could not be read and quotes its first line; the exported
  log keeps the full text.
- **Replies through Proton Bridge join their conversation.** Message-IDs
  are stored lowercased so threads match however a client spelt an id, and
  a reply carried those lowercased ids in In-Reply-To and References.
  Bridge looks the parent up by its exact id, found nothing, and filed the
  sent copy with no threading headers. Every exact spelling seen while
  parsing is now kept (`msgid_case`) and a message being sent, queued or
  saved as a draft gets its ids spelt back the way they arrived.
- **The Contacts page says "No Contacts"** when there is no address book,
  instead of "Loading Contacts…" forever. Both strings are translatable.
- **Settings → Mail Accounts draws the Gravatar** in the account circles,
  as the sidebar and the editor preview already did.
- **The theme gallery draws each theme as a small window**, in the style
  of the GNOME Settings appearance chooser: header bar, sidebar with its
  selected row, content pane with a card and an accent button, light on
  the left and dark on the right. The System card reads the desktop's
  accent colour and follows it when it changes.
- **New icon and wordmark art.** The default icon, its development twin
  and the full-size gallery entry come from fresh sources, and the wizard
  and About window draw the wordmark from a PNG master, since neither
  librsvg nor gdk-pixbuf renders the mark's soft shadow.
- **Themed dividers read as shadows.** The sidebar and rail divider uses
  libadwaita's own border shade, and the list/reader divider is painted a
  fifth darker, so neither reads as a bright stripe on a themed window.
- **Greek translation** (PR #227 by @yioannides, with the bulk of the
  strings generated by @somepaulo): 1234 of 1238 strings.
