...
- **Reader View** — one toggle in the reading pane's header shows every message in the open conversation as its content alone: the sender's layout tables, colours, fonts, hidden preview text and tracking pixels are stripped, and what is left (headings, paragraphs, lists, quotes, links, real pictures and data tables) is set in one uniform sheet that follows the app's theme. Off again, mail is shown as sent.
